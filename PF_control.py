...
def image_callback(data):
    ...
    im_rgb = bridge.imgmsg_to_cv2(data, desired_encoding="passthrough")
    #imagem = data.data
    # flag = 1
...
def depth_callback(data):
    ...
    global imagem, bridge, flag
    imagem = bridge.imgmsg_to_cv2(data, desired_encoding="passthrough")
    #imagem = data.data
    flag = 1

def point_callback(data):
    global pointc, x,y 
    point = pc2.read_points(data, skip_nans=True, field_names=("x", "y", "z"))
    #x = np.array([a[0] for a in point]).reshape((480,640))
    #y = np.array([a[1] for a in point]).reshape((480,640))
    pointc = np.array([a[2] for a in point]).reshape((480,640))
    #pointc = np.array([a[2] for a in point]).reshape((480,640))
    #Axes3D.scatter(x, y, pointc)
...
def preprocessing():
    ...
    imagem_corrigida[imagem_corrigida<1.5] = 1.5

    img = normalize(imagem_corrigida).astype(np.uint8)
    yf, xf = img.shape
    x = 150
    ...
    im_rgba = deepcopy(im_rgb[200:yf, x:xf-x,:])
    ...
    return img, imagempp, imga, im_rgba
...
def run_control():
    ...
    pf = ParticleFilter(transition = transition, fitness = fitness, nfeatures=4, 
        mu = [0,0,50,100], sigma = [1,10,50,50], lb = [-np.pi/2, -max_lateral_offset, 0, 0], 
...
def main():
    ...
    rospy.Subscriber("/soybot/center_camera/depth/image_raw", Image, depth_callback)  
    rospy.Subscriber("/soybot/center_camera/color/image_raw", Image, image_callback)  
	#rospy.Subscriber("/soybot/center_camera/depth/points", PointCloud2, point_callback)   
    bridge = CvBridge()
    run_control()
